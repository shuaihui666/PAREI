...
def BM25Process():
    ...
    testIndexItem=0
    Res=[]
    for testIndex in test_mashup_id:
        bmDesArray=[]
        tempIndex=[]
        Res.append([])
...
def simCsePro(test_mashup_id,rightApi,embeddings,rightMashupScore):
    os.makedirs(resPath, exist_ok=True)
    leftId=test_mashup_id
    rightId=rightApi
    Res=[]
    IndexLeft=0
    for leftItem in leftId:
        ...
        # print(str(IndexLeft)+"/"+str(len(test_mashup_id)))
        ...
        print(str(IndexLeft+1)+"/"+str(len(test_mashup_id))+' Running time: '+str(end-start)+' Seconds')
...
def ListRebuild2(BM25_Scores):
    ...
    for line in tempFile:
        Res.append([])
        Res[Index].append(int(line.split()[0]))
        Res[Index].append(" ".join(line.split()[1:]))
        Index = Index + 1
    tempFile.close()
    Res.sort()
    tmp_fd, tmp_path = tempfile.mkstemp(suffix='.txt')
    with os.fdopen(tmp_fd, 'w') as f:
        f.write(str(len(mashup_id)) + " " + str(100) + "\n")
        for line in mashup_id:
            f.write(str(Res[line][0]) + " " + Res[line][1] + "\n")
    # 从临时文件加载筛选后的mashup嵌入
    node_embeddings = gensim.models.KeyedVectors.load_word2vec_format(tmp_path, binary=False)
    os.unlink(tmp_path)
    Index=0
    Res=[]
    MashupScoreRes=[]
    for testIndexItem in BM25_Scores:
        ...
        for topNItem in range(0,trussTopN):
            ...
            for nodeItem in nodeSimRes:
                if((int(nodeItem[0]) not in topNodeList) and int(nodeItem[0])!=test_mashup_id[Index]):
    ...
    for topn in topNNum:
        ...
        for item in testMashupId:
            ...
            for line in file.readlines():
                ...
                rightDataTemp[Index].append(float(line.split()[3]))
                Index=Index+1
            rightDataTemp.sort(key=lambda x:x[0],reverse=True)#倒序排序
            tempDict={}
            for ndItem in rightDataTemp:
                if(ndItem[0] in tempDict.keys()):
                    tempDict[ndItem[0]]=tempDict[ndItem[0]]+ndItem[1]*ndItem[2]
                else:
                    tempDict[ndItem[0]]=ndItem[1]*ndItem[2]
            # print(tempDict)
            rightData=[]
            keyIndex=0
...
if __name__=="__main__":
    ...
    simQA=simCsePro(test_mashup_id,rightApi,ems,rightMashupScore)
    #获取simcse相似度比较结果

    evalData(test_mashup_id)
