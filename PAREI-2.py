...
def sim(eval_mashup_id,rightApi,embeddings,rightMashupScore):
    ...
    for leftItem in leftId:
        ...
        bmDesArray=[]
        Res.append([])
        for textId in rightId[IndexLeft]:
        ...
        for rightItem in rightId[IndexLeft]:
            ...
            file.write(str(rightItem)+" "+str(single_score)+' '+str(rightMashupScore[IndexLeft][IndexRight][0])+' '+str(rightMashupScore[IndexLeft][IndexRight][1])+'\n')
...
if __name__=="__main__":
    ...
    simQA=sim(eval_mashup_id,rightApi,ems,rightMashupScore)
    #获取simcse相似度比较结果
